...
- **講師後台 = `analytics.html`**（→ redirect `engine/analytics-template.html`，config 走 sessionStorage）。三個必備：(1) index.html 的 `#tqeTeacherLink` href 要指 `analytics.html`（原本 `#` 沒接）；(2) analytics.html 的 `firebaseConfig` 要填（原本空 `{}` 連不上）；(3) **RTDB rules 要給講師 email 讀取例外**——後台讀 `wellap/users_auth`（全體學生）+ `wellap` root，但 per-uid 規則擋住，需在 `wellap` 層加 `.read: auth.token.email === '<teacher>'`（學生 email 不符 → 退回只能讀自己，安全不破）。teacher email 在 content.js `teacherEmail` + analytics.html。2026-06-04 踩過。
